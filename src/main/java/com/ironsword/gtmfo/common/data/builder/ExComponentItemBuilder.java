...
public abstract class ExComponentItemBuilder<A extends Item, T extends ExComponentItemBuilder<A, T>>
                                            extends ItemBuilder<A, T> {
    ...
    private static final class Impl extends ExComponentItemBuilder<ExComponentItem, Impl> {
        ...
        public com.tterrag.registrate.builders.ItemBuilder<ExComponentItem, GTRegistrate> build() {
            ...
            if (_CnLang != null)
                builder.setData(GTMFOProviderTypes.CNLANG,
                        (ctx, prov) -> prov.add(ctx.get().getDescriptionId(), _CnLang));

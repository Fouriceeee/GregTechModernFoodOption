...
public class GTMFOModels {
    ...
    public static <
            T extends FruitBushBlock> NonNullBiConsumer<DataGenContext<Block, T>, RegistrateBlockstateProvider> fruitBushModel(ResourceLocation flowerOverlay,
        ...
        return (ctx, prov) -> {
            prov.getVariantBuilder(ctx.getEntry())
                    .forAllStates(state -> {
                        ...
                        ModelFile parent = prov.models().getExistingFile(GregTechModernFoodOption
                                .id("block/parent/fruit_bush/%s".formatted(direction.getName())));
                        ModelBuilder<?> model = prov.models().getBuilder(
                                "block/fruit_bush/%s/stage_%d/%s".formatted(ctx.getName(), age, direction.getName()))
                                .parent(parent);

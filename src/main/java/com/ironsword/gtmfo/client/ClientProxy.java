...
public class ClientProxy extends CommonProxy {
    ...
    public void onRegisterBlockColors(RegisterColorHandlersEvent.Block event) {
        event.register(
                ((pState, pLevel, pPos, pTintIndex) -> ((FruitBushBlock) pState.getBlock())
                        .getOverlayColor(pState.getValue(FruitBushBlock.AGE))),
                ForgeRegistries.BLOCKS.getValues().stream().filter(FruitBushBlock.class::isInstance)
                        .toArray(Block[]::new));
    }
